def precision(x_pred, x_true):
    return len([x for x in x_pred if x in x_true]) / len(x_pred)


def recall(x_pred, x_true):
    return len([x for x in x_pred if x in x_true]) / len(x_true)


def average_precision(x_pred, x_true):
    return sum(precision(p, t) for p, t in zip(x_pred, x_true)) / len(x_pred)


def average_recall(x_pred, x_true):
    return sum(recall(p, t) for p, t in zip(x_pred, x_true)) / len(x_pred)
...
def average_f_score(x_pred, x_true):
    ...
    return sum(f_scores) / len(x_pred)
